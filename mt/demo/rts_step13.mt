// RTS step 13 — impassable terrain + tree/rock decorations.
//
// Builds on rts_step12:
//   * Water (deep + shallow) and Mountain tiles now block movement.
//     A `passable` mask is derived from biome at startup.
//   * A* pathfinding (4-connected, Manhattan heuristic, binary-heap
//     open set) routes units around obstacles. Each unit holds its
//     own waypoint list in a flat `pathTiles` buffer; the movement
//     integrator follows the path one waypoint at a time.
//   * Decorations: trees scattered in BIO_FOREST tiles, rocks in
//     BIO_STONE tiles. Placement is deterministic (hash-based) and
//     they render between the ground and the units, fog-gated like
//     resource nodes.
//
// Everything else from step 12 (biomes, audio, buildable barracks,
// queues, save/load, fog of war, minimap) carries over.
...
// ---- pathfinding ----
int MAX_PATH = 128;       // upper bound on per-unit waypoint count
int aMax     = MAP_TILES * MAP_TILES;
int A_INF    = 999999;    // "infinity" for g/fScore
...
class Unit {
    ...
    public int   pathLen;     // 0 if no path (use direct tx/ty)
    public int   pathStep;    // index into the unit's path slice
...
// ---- window ----
RenderWindow window = Sfml::createWindow("RTS — step 13", WIN_W, WIN_H);
window.setFramerateLimit(144);

// ---- biome generation ----
int[] biome = new int[MAP_TILES * MAP_TILES];

float NOISE_FREQ   = 0.075;
float NOISE_ORIGIN = 137.0;
...
// Buildings + nodes (defined here so we can carve land around them).
...
// Hall + node land-carving (same as step 12).
...
// ---- passability mask derived from biome ----
//
// Water (deep + shallow) and Mountain block movement. Everything else
// is walkable.
int[] passable = new int[MAP_TILES * MAP_TILES];
for (int c = 0; c < MAP_TILES * MAP_TILES; c++) {
    int b = biome[c];
    if (b == BIO_DEEP_WATER || b == BIO_WATER || b == BIO_MOUNTAIN) {
        passable[c] = 0;
    } else {
        passable[c] = 1;
    }
...
// ---- decorations (forest trees + stone rocks) ----
int     decoCount = 0;
float[] decoX     = new float[MAX_DECO];
float[] decoY     = new float[MAX_DECO];
int[]   decoTypeArr = new int[MAX_DECO];
for (int ty = 0; ty < MAP_TILES; ty++) {
    for (int tx = 0; tx < MAP_TILES; tx++) {
        if (decoCount >= MAX_DECO) { continue; }
        int b = biome[ty * MAP_TILES + tx];
        int h = hash2i(tx + 1000, ty - 1000) % 100;
        int placeTree = 0;
        int placeRock = 0;
        if (b == BIO_FOREST && h < 38) { placeTree = 1; }
        if (b == BIO_STONE  && h < 22) { placeRock = 1; }
        if (placeTree == 1 || placeRock == 1) {
            int xh = hash2i(tx + 500, ty) % TILE_PX;
            int yh = hash2i(tx, ty + 500) % TILE_PX;
            decoX[decoCount] = (float)(tx * TILE_PX) + (float)xh;
            decoY[decoCount] = (float)(ty * TILE_PX) + (float)yh;
            if (placeTree == 1) { decoTypeArr[decoCount] = DECO_TREE; }
            else                  { decoTypeArr[decoCount] = DECO_ROCK; }
...
// ---- per-unit path storage (flat) + global path-find buffers ----
int[] pathTiles = new int[MAX_UNITS * MAX_PATH];

int[] globalPath = new int[MAX_PATH];
int   globalPathLen = 0;

int[] aGScore   = new int[aMax];
int[] aFScore   = new int[aMax];
int[] aCameFrom = new int[aMax];
int[] aClosed   = new int[aMax];
int[] aHeap     = new int[aMax];
int   aHeapCount = 0;

// Min-heap keyed by aFScore[tile]. Clean break-out — the previous
// `i = 0; i = -1; if (i == -1) break;` sentinel trick triggered a JIT
// crash after warm-up.
...
// 4-connected A*. Fills globalPath[0..len-1] with tile indices in
// start-to-goal order. Returns the path length, 0 if no path.
function aStar(int sx, int sy, int gx, int gy): int {
    ...
    int start = sy * MAP_TILES + sx;
    int goal  = gy * MAP_TILES + gx;
    aGScore[start] = 0;
    aFScore[start] = manhattan(sx, sy, gx, gy);
    heapPush(start);

    int foundGoal = 0;
    while (aHeapCount > 0 && foundGoal == 0) {
        int curr = heapPop();
        if (curr == goal) {
            foundGoal = 1;
        } else if (aClosed[curr] == 0) {
            aClosed[curr] = 1;
            int cx = curr - (curr / MAP_TILES) * MAP_TILES;
            int cy = curr / MAP_TILES;
            for (int d = 0; d < 4; d++) {
                int nx = cx; int ny = cy;
                if      (d == 0) { nx = cx + 1; }
                else if (d == 1) { nx = cx - 1; }
                ...
                int n = ny * MAP_TILES + nx;
                if (passable[n] == 0) { continue; }
                if (aClosed[n] == 1)   { continue; }
                int tentativeG = aGScore[curr] + 1;
                if (tentativeG < aGScore[n]) {
                    aCameFrom[n] = curr;
                    aGScore[n] = tentativeG;
                    aFScore[n] = tentativeG + manhattan(nx, ny, gx, gy);
                    heapPush(n);
                }
            }
        }
    ...
    // Count path length by walking parents.
    int len = 0;
    int counter = goal;
    while (counter != -1) {
        len = len + 1;
        counter = aCameFrom[counter];
    }
...
// Convenience: assign `goalX/goalY` to unit `i`, computing a path
// through `passable`. If the goal is impassable, the nearest passable
// tile (within radius 10) is used. If no path is reachable, falls back
// to a straight-line target so the unit still moves toward something.
function planUnitPath(int unitIdx, float goalX, float goalY): void {
    int sx = (int)(units[unitIdx].x / (float)TILE_PX);
    int sy = (int)(units[unitIdx].y / (float)TILE_PX);
    int gx = (int)(goalX / (float)TILE_PX);
    int gy = (int)(goalY / (float)TILE_PX);
    if (sx < 0) { sx = 0; } if (sx >= MAP_TILES) { sx = MAP_TILES - 1; }
    if (sy < 0) { sy = 0; } if (sy >= MAP_TILES) { sy = MAP_TILES - 1; }
    if (gx < 0) { gx = 0; } if (gx >= MAP_TILES) { gx = MAP_TILES - 1; }
    if (gy < 0) { gy = 0; } if (gy >= MAP_TILES) { gy = MAP_TILES - 1; }

    if (passable[gy * MAP_TILES + gx] == 0) {
        int found = 0;
        for (int r = 1; r < 10; r++) {
    ...
    // If we're already on the goal tile, no path needed.
    if (sx == gx && sy == gy) {
        units[unitIdx].pathLen = 0;
        units[unitIdx].pathStep = 0;
        units[unitIdx].tx = goalX;
        units[unitIdx].ty = goalY;
        units[unitIdx].hasTarget = 1;
        return;
    }

    int len = aStar(sx, sy, gx, gy);
    units[unitIdx].pathLen = len;
    units[unitIdx].pathStep = 0;
    if (len > 0) {
        for (int k = 0; k < len; k++) {
            pathTiles[unitIdx * MAX_PATH + k] = globalPath[k];
        }
        int firstTile = pathTiles[unitIdx * MAX_PATH + 0];
        int wx = firstTile - (firstTile / MAP_TILES) * MAP_TILES;
        int wy = firstTile / MAP_TILES;
        units[unitIdx].tx = (float)wx * (float)TILE_PX + (float)TILE_PX * 0.5;
        units[unitIdx].ty = (float)wy * (float)TILE_PX + (float)TILE_PX * 0.5;
    } else {
        units[unitIdx].tx = goalX;
        units[unitIdx].ty = goalY;
    }
...
int   sepCellCount = SEP_GRID_W * SEP_GRID_H;
int[] cellHead  = new int[sepCellCount];
...
// Decoration shapes — reused for every tree/rock.
CircleShape treeShape = Circles::create(6.5);
treeShape.setOrigin(6.5, 6.5);
treeShape.setFillColor(22, 65, 22, 255);
treeShape.setOutlineColor(10, 30, 10, 255);
treeShape.setOutlineThickness(1.0);

CircleShape rockShape = Circles::create(4.5);
rockShape.setOrigin(4.5, 4.5);
rockShape.setFillColor(120, 120, 125, 255);
rockShape.setOutlineColor(60, 60, 65, 255);
rockShape.setOutlineThickness(1.0);
...
Text txtHint = Texts::create(hudFont,
    "Water + mountains block movement — units route around   F5/F9: save/load   Esc: quit",
    13);
...
while (window.isOpen()) {
        ...
        if (ev == evClosed) {
            ...
            if (mb == mbLeft) {
                ...
                if (placingBarracks == 1) {
                    ...
                    // Don't allow placement on impassable tiles.
                    int bx = (int)(wx / (float)TILE_PX);
                    int by = (int)(wy / (float)TILE_PX);
                    if (bx >= 0 && by >= 0 && bx < MAP_TILES && by < MAP_TILES) {
                        if (passable[by * MAP_TILES + bx] == 0) { valid = 0; }
                    }
                        ...
                        if (hitEnemy >= 0) {
                            for (int i = 0; i < unitCount; i++) {
                                if (units[i].selected == 1 && units[i].type == UT_SOLDIER) {
                                    ...
                                    planUnitPath(i, enemies[hitEnemy].x, enemies[hitEnemy].y);
                                }
                            }
                            int wkCount = 0;
                            for (int i = 0; i < unitCount; i++) {
                            ...
                            if (wkCount > 0) {
                                ...
                                for (int i = 0; i < unitCount; i++) {
                                    if (units[i].selected == 1 && units[i].type == UT_WORKER) {
                                        ...
                                        planUnitPath(i,
                                            ox + (float)sx * FORM_SPACING,
                                            oy + (float)sy * FORM_SPACING);
                                        slot = slot + 1;
                                    }
                                }
                            }
                            ...
                            for (int i = 0; i < unitCount; i++) {
                                if (units[i].selected == 1 && units[i].type == UT_WORKER) {
                                    ...
                                    if (units[i].cargo > 0) {
                                        ...
                                        planUnitPath(i, HALL_X, HALL_Y);
                                    } else {
                                        units[i].state = WS_TO_RES;
                                        planUnitPath(i, nodes[hitNode].x, nodes[hitNode].y);
                                    }
                                }
                            }
                            int solCount = 0;
    ...
    // F5 / F9 save/load (unchanged from step 12).
    int f5Now = 0;
    ...
    if (f9Now == 1 && f9Latch == 0 && hasSnapshot == 1) {
        ...
        for (int i = 0; i < unitCount; i++) {
            ...
            // Reset path so the unit re-plans from its restored position.
            units[i].pathLen  = 0;
            units[i].pathStep = 0;
            // If the unit had a target, replan a fresh path to it.
            if (units[i].hasTarget == 1) {
                planUnitPath(i, units[i].tx, units[i].ty);
            }
        }
    ...
    // FSM. Transitions that retarget a unit now call planUnitPath so
    // the path follows the new goal — but only on the transition, not
    // every frame (so we don't re-A* during chase).
    ...
    for (int i = 0; i < unitCount; i++) {
        ...
        if (s == WS_TO_RES) {
            ...
            if (dx * dx + dy * dy <= depositR2) {
                ...
                if (k >= 0 && nodes[k].amount > 0) {
                    units[i].state = WS_TO_RES;
                    planUnitPath(i, nodes[k].x, nodes[k].y);
                } else {
                    units[i].state = WS_IDLE;
                    units[i].assignedNode = -1;
                    units[i].hasTarget = 0;
            ...
            if (eIdx < 0 || enemies[eIdx].hp <= 0) {
                ...
                // No re-plan while chasing — path was set at order time
                // and enemies don't move, so the existing path is fine.
            }
            ...
            if (eIdx < 0 || enemies[eIdx].hp <= 0) {
                ...
                if (d2 > atkR2) {
                    units[i].state = SOL_TO_ENEMY;
                    planUnitPath(i, enemies[eIdx].x, enemies[eIdx].y);
                } else {
    ...
    // Path-following movement integrator. When the unit reaches its
    // current waypoint (tx, ty), the next tile in pathTiles becomes
    // the new tx, ty. When the path is exhausted, hasTarget = 0.
    for (int i = 0; i < unitCount; i++) {
        if (units[i].hasTarget == 1) {
            float ux = units[i].x;
            float uy = units[i].y;
            ...
            if (dist <= ARRIVE_EPS || move >= dist) {
                units[i].x = units[i].tx;
                units[i].y = units[i].ty;
                int nextStep = units[i].pathStep + 1;
                if (units[i].pathLen > 0 && nextStep < units[i].pathLen) {
                    units[i].pathStep = nextStep;
                    int nextTile = pathTiles[i * MAX_PATH + nextStep];
                    int wx = nextTile - (nextTile / MAP_TILES) * MAP_TILES;
                    int wy = nextTile / MAP_TILES;
                    units[i].tx = (float)wx * (float)TILE_PX + (float)TILE_PX * 0.5;
                    units[i].ty = (float)wy * (float)TILE_PX + (float)TILE_PX * 0.5;
                } else {
                    units[i].hasTarget = 0;
                    units[i].pathLen = 0;
                }
    ...
    // Separation via spatial grid (unchanged from step 12).
    for (int i = 0; i < unitCount; i++) { sepDX[i] = 0.0; sepDY[i] = 0.0; }
    for (int c = 0; c < sepCellCount; c++) { cellHead[c] = -1; }
    ...
    for (int i = 0; i < unitCount; i++) {
        // Apply separation only if the next position stays on passable
        // terrain — keeps separation forces from pushing a unit into
        // water/mountain.
        float nx = units[i].x + sepDX[i] * sepStep / SEP_RADIUS;
        float ny = units[i].y + sepDY[i] * sepStep / SEP_RADIUS;
        int tcx = (int)(nx / (float)TILE_PX);
        int tcy = (int)(ny / (float)TILE_PX);
        int ok = 1;
        if (tcx < 0 || tcy < 0 || tcx >= MAP_TILES || tcy >= MAP_TILES) { ok = 0; }
        else if (passable[tcy * MAP_TILES + tcx] == 0) { ok = 0; }
        if (ok == 1) {
            units[i].x = nx;
            units[i].y = ny;
        }
    ...
    // Decorations — render between ground and units. Fog-gated.
    for (int i = 0; i < decoCount; i++) {
        int cx = (int)(decoX[i] / FOG_CELL);
        int cy = (int)(decoY[i] / FOG_CELL);
        int seen = 0;
        if (cx >= 0 && cy >= 0 && cx < FOG_W && cy < FOG_H) {
            if (fog[cy * FOG_W + cx] >= FOG_EXPLORED) { seen = 1; }
        }
        if (seen == 1) {
            if (decoTypeArr[i] == DECO_TREE) {
                treeShape.setPosition(decoX[i], decoY[i]);
                Draw::circle(window, treeShape);
            } else {
                rockShape.setPosition(decoX[i], decoY[i]);
                Draw::circle(window, rockShape);
            }
        }
    }
